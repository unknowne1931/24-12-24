    ...
    try {
        ...
        if (!userByEmail && !userByUsername) {
            ...
            bcrypt.hash(pass, 10).then(async (hash) => {
                ...
                return res.status(200).json({ Status: "ERR_HASH", message: "Password encryption failed." });
            });
        } else {
...
app.post('/get/all/users/data/otp/to/verify/02', async (req, res) => {
    ...
    try {
        ...
        if (find_user && find_user.otp === OTP) {
            ...
            const token = jwt.sign({ id : user._id }, "kanna_staWro_founders_withhh_1931_liketha", { expiresIn: "4h" });
